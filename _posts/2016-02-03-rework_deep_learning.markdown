...
There is clearly a lot of excitement about this area of research for its applications, my thoughts about the companies at this conference is that they can be generally divided into the following categories.  The fairly large ones (Pinterest, Twitter, eBay, Flickr) use DL to be competitive.  It is another tool in the bag of various other statistical/machine learning techniques they were already using, but works particularly well with the deluge of visual data and natural language processing tasks they must do at scale.  Of the smaller companies, some with medical applications seemed like they had both a great business model and a tractable problem (see for instance BayLabs below for ultrasound diagnosis).  These companies mainly take advantage of the advances in computer vision to do classification, which can make diagnosis both more efficient more accessible.  This seems convincing as long as there is a well labelled set for the problem, and in the case of ultrasound diagnosis, partnering with hospitals and research institutions seems to provide just that. Furthermore in terms of responsibility, it can only be an aide and not replacement for doctors.  There were also two companies focusing on the analysis of satellite images.  One can imagine how valuable this data is once obtained (counting world oil supplies is one good example, as well as measuring construction activity in China as a proxy for the Chinese economy).  Finally there were some more consumer facing verticals that try to use DL to improve the personalization aspect of the interaction (for instance in shopping and recommendations, organizing your photo library...etc).  
